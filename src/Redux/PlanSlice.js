...
export const GetFixedPlannData = createAsyncThunk("api/fixedPlan",async(_,thunkAPI)=>{
  const resss = await axios.get('https://localhost:7152/api/Plan/GetFixedPlans?num=5')
  .then(function (response) {
...
PlanCommendData : {
  ...
  GetPlanById : (state) =>{
    ...
    axios.get('https://localhost:7152/api/Plan/Return plan?id=2')
.then(function (response) {
...
GetPlanByName : (state) =>{
    ...
    axios.get('https://localhost:7152/api/Plan/GetPlanByName?name=name')
.then(function (response) {
  const res = response.data
  console.log(res);
})
...
export const {pushPlanPayload,pushToArray,TakeExecuteLaterEnable,TakeTimeLater,TakeRepeat,TakeDelay,TakePlanName,TakeSubsystemItem,TakeCommendItem,nowDate,GetPlanData,GetPlanTypeData,GetPlanById,GetPlanByName} = PlanSlice.actions
